...
def get_dc_proba(log_lh, log_la):
...
def get_elo_proba(rh, ra, h_adv=60.0, draw_param=0.28):
    d_elo = (rh + h_adv) - ra
    ...
    tot = p_h + p_d + p_a
    return p_h / tot, p_d / tot

def generate_multi_model_dataset(n_sims=2000):
    ...
    models = ['Dynamic Dixon-Coles (V2)', 'Dynamic Elo (V1)', 'Static Poisson (Baseline)']
    records = []
    ...
    for model_name in models:
        ...
        for s_idx, target_s in enumerate(all_seasons):
            ...
            for md in range(0, max_md + 1):
                if md > 0:
                    ...
                    for m in md_matches:
                        ...
                        # Post-match ratings update
                        if model_name == 'Dynamic Dixon-Coles (V2)':
                        ...
                        for r_md, r_fixtures in rem_by_md.items():
                            ...
                            for h, a in r_fixtures:
                                ...
                                if model_name == 'Dynamic Dixon-Coles (V2)':
                                    ...
                                    ph, pd_ = get_dc_proba(log_lh, log_la)
                                    if rnd < ph:
                                    ...
                                    rh = s_elo.get(h, 1500.0)
                                    ra = s_elo.get(a, 1500.0)
                                    ph, pd_ = get_elo_proba(rh, ra)
                                    if rnd < ph:
                                        s_pts[h] += 3; s_gd[h] += 1; s_gd[a] -= 1
                                        s_act = 1.0
                                    elif rnd < ph + pd_:
                                        ...
                                        s_pts[a] += 3; s_gd[a] += 1; s_gd[h] -= 1
                                        s_act = 0.0
                                    e_h = 1.0 / (1.0 + math.pow(10.0, -((rh + 60.0) - ra) / 400.0))
                                    s_elo[h] = rh + 15.0 * (s_act - e_h)
                                    s_elo[a] = ra - 15.0 * (s_act - e_h)
                                else: # Static Poisson
                                    ph, pd_ = 0.44, 0.26
                                    if rnd < ph:
                                        s_pts[h] += 3; s_gd[h] += 1; s_gd[a] -= 1
            ...
            beta  = dict(curr_beta)
            elo_ratings = dict(curr_elo)

    df_out = pd.DataFrame(records)
    out_path = os.path.join(os.path.dirname(__file__), "nbi_multi_model_matchday_probabilities.csv")
    df_out.to_csv(out_path, index=False, encoding='utf-8-sig')
    print(f"\n[+] Success! Generated {len(df_out)} multi-model records.")
    print(f"[+] Saved dataset to: {out_path}")

if __name__ == "__main__":
    generate_multi_model_dataset()
