...
def scrape_sofascore(url):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"Error: {resp.status_code} for {url}", flush=True)
            return {}
...
def create_lineups_df(event_id):
    print(f"create_lineups_df starting for {event_id}", flush=True)
    home_players = []
    away_players = []
...
def create_average_positions_df(event_id):
    ...
    # Process home players
    for player in average_positions_data.get('home', []):
        player_info = player['player'].copy()
        player_info.update({
    ...
    # Process away players
    for player in average_positions_data.get('away', []):
        player_info = player['player'].copy()
        player_info.update({
            'averageX': player['averageX'],
            'averageY': player['averageY'],
...
def fetch_match_details(event_id):
    """Fetches general match information (teams, scores, tournament, date)."""
...
def fetch_player_heatmap(event_id, player_id):
    """Fetches heatmap data for a specific player in an event."""
...
def fetch_attacking_momentum(event_id):
    """Fetches attacking momentum data points."""
    url = f"https://www.sofascore.com/api/v1/event/{event_id}/graph"
    data = scrape_sofascore(url)
    if not data or 'graphPoints' not in data:
        return []
