...
TEAM_URL = "https://fbref.com/en/squads/6611f992/2024-2025/matchlogs/all_comps/shooting/Ferencvaros-Match-Logs-All-Competitions"
PLAYER_URL = "https://fbref.com/en/players/078a52fe/matchlogs/2024-2025/Barnabas-Varga-Match-Logs"
...
merged = team_df.merge(
    player_df,
    left_on=["For Ferencváros_Date", "For Ferencváros_Opponent"],
    right_on=["Date", "Opponent"],
    how="left",   # így a játékos nélküli meccsek is megmaradnak
    suffixes=("_team", "_player")
)

merged["Varga_played"] = merged["Min"].fillna(0).astype(float) >= 30
merged = merged[pd.notna(merged["For Ferencváros_Opponent"])]

# get datatypes right
merged["For Ferencváros_GF"] = merged["For Ferencváros_GF"].str.split("(").str[0].str.strip().astype(float)
merged["For Ferencváros_GA"] = merged["For Ferencváros_GA"].str.split("(").str[0].str.strip().astype(float)
...
merged["Points"] = merged["For Ferencváros_Result"].apply(result_to_points)


summary = merged.groupby("Varga_played").agg({
...
renamed = merged.rename(columns={
    "For Ferencváros_GF": "Goals For",
    "For Ferencváros_GA": "Goals Against",
    "Standard_Sh": "Shots",
...
summary = renamed.groupby("Varga_played").agg({
    "Goals For": "mean",
    "Goals Against": "mean",
    "Shots": "mean",
...
ax.set_xticklabels(["Without Varga", "With Varga"], color='white')
ax.set_xlabel("")
ax.set_ylabel("Average per Match", color='white')
ax.set_title("Ferencváros Goals For and Against (per match)", color='white')
...
plt.savefig(f"{save_folder}\\WWYI_goals_vs_against.png", dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.show()
...
ax.set_title("Ferencváros Points per Match With and Without Varga", color='white')
ax.spines['bottom'].set_color('white')
ax.spines['left'].set_color('white')
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
plt.savefig(f"{save_folder}\\WWYI_points_per_match.png", dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.show()
...
axes[0].set_xticklabels(["Without Varga", "With Varga"], color='white')
axes[0].set_xlabel("")
...
axes[1].set_xticklabels(["Without Varga", "With Varga"], color='white')
axes[1].set_xlabel("")
axes[1].set_ylabel("Percentage of Shots on Target", color="white")
axes[1].set_title("Shot Accuracy (SoT per Shot)", color='white')

plt.suptitle("Ferencváros Shooting Metrics With and Without Varga", fontsize=16, color='white')

fig.text(0.9, 0.96, 'ADAM JAKUS', color=mycolor, fontsize=16, fontproperties=my_font_props,
         ha='center', va='center')

plt.tight_layout()
plt.savefig(f"{save_folder}\\WWYI_shooting_metrics.png", dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.show()
...
ax.set_title("Ferencváros Goal Difference Over the Season (Colored by Varga presence)", color='white')
ax.spines['bottom'].set_color('white')
ax.spines['left'].set_color('white')
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
plt.savefig(f"{save_folder}\\WWYI_goal_difference_scatter.png", dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.show()
