...
#%%
print(df_comparison.isIn_interval.sum()/len(df_comparison))
...
"""
###df_errors = pd.DataFrame({'season3': season3,
                         'avg_error': [np.average(df_comparison.abs_PtsDiff)],
                         'first_quart_error':[np.quantile(df_comparison.abs_PtsDiff, 0.25)],
                         'median_error':[np.quantile(df_comparison.abs_PtsDiff, 0.5)],
                         'third_quart_error':[np.quantile(df_comparison.abs_PtsDiff, 0.75)]})
"""
df_errors = pd.concat([df_errors,pd.DataFrame({'season3': season3,
                         'avg_error': [np.average(df_comparison.abs_PtsDiff)],
                         'first_quart_error':[np.quantile(df_comparison.abs_PtsDiff, 0.25)],
                         'median_error':[np.quantile(df_comparison.abs_PtsDiff, 0.5)],
                         'third_quart_error':[np.quantile(df_comparison.abs_PtsDiff, 0.75)]})])

df_errors.to_excel(r'C:\TwitterSportsDataProject\fbref_scrape_errors.xlsx')
