...
TEAM_URL = "https://fbref.com/en/squads/e2d8892c/2024-2025/matchlogs/c13/possession/Paris-Saint-Germain-Match-Logs-Ligue-1"
PLAYER_URL = "https://fbref.com/en/players/c2a15a27/matchlogs/2024-2025/Joao-Neves-Match-Logs"
...
for i in player_df.index:
    for col in player_df.columns:
            if "On matchday" in str(player_df.loc[i, col]):
                player_df.loc[i, col] = 0
...
team = [col for col in team_df.columns if 'For ' in col][0].split('_')[0].split('For ')[-1]
print(team)

#%% analysis
merged = team_df.merge(
    player_df,
    left_on=[f"For {team}_Date", f"For {team}_Opponent"],
...
merged = merged[pd.notna(merged[f"For {team}_Opponent"])]

# get datatypes right
merged[f"For {team}_GF"] = merged[f"For {team}_GF"].astype(float)
merged[f"For {team}_GA"] = merged[f"For {team}_GA"].astype(float)
...
merged["Points"] = merged[f"For {team}_Result"].apply(result_to_points)
